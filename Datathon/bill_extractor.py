...
def detect_header_and_boundaries(rows):
    ...
    for x, word in header:
        ...
        if "desc" in w:
            ...
            x_rate = x
        elif "net" in w or "gross" in w or "amount" in w:
            x_amount = x
...
def extract_items(rows, header_idx, borders):
    ...
    for row in rows[header_idx + 1:]:
        ...
        desc = remove_slno(" ".join(desc_bucket).strip())
        qty = to_float("".join(qty_bucket))
        rate = to_float("".join(rate_bucket))
...
def extract_bill_info_from_url(url: str) -> dict:
    """
    Main orchestrator: download → OCR → row grouping → parse items
    ...
    return {
        ...
        "data": {
            "pagewise_line_items": [
                {
                    "page_no": "1",
                    "page_type": "Bill Detail",   # static now; can be improved later
                    "bill_items": items
                }
            ],
            "total_item_count": len(items)
